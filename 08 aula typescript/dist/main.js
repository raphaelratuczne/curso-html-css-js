...
let meuNumero = 0;
let meuBool = true;
let varVariavel = "qualquer coisa";
const meuArray = ["Paula", "Nati"];
const meuSecArray = [1, 2, 3, 4];
const meuTerArray = [];
const meuObj = {
    nome: "Raphael",
    altura: 1.71,
...
function teste({ outraVar = 100, msg }) {
    const msgCriada = `a msg é ${msg}, ${outraVar}`;
    console.log(msgCriada);
    return msgCriada;
}
function teste2({ msg, finalizado }) {
    if (finalizado) {
...
if (searchParams.has("idade")) {
    console.log("idade", searchParams.get("idade"), typeof searchParams.get("idade"));
}
