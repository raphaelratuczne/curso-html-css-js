...
import { deleteUser, loadUsers } from './apis/usuarios';
...
btnConfirmExclude.addEventListener('click', async () => {
  if (excludeUserId) {
    ...
    enderecos.forEach(async end => {
      await deleteEndereco(end.id);
    });
    // carrega os documentos do usuario
    const documentos = await getDocumentosByUser(excludeUserId);
    documentos.forEach(async doc => {
      await deleteDocumento(doc.id);
    });
    await deleteUser(excludeUserId);
    excludeUserId = '';
  }
  modal.close();
  init();
